...
const moderatorViewer: HookdUser = {
  uid: "moderator-uid",
...
const baseBundle: ForecastBundle = {
  version: "2024-10-05",
...
test("ForecastPanel surfaces provider attribution and manual refresh analytics for moderators", async () => {
...
test("ForecastPanel hides provider alerts for non-moderators", async () => {
  const dom = new JSDOM("<!doctype html><html><body></body></html>", { url: "http://localhost" });
  const originalWindow = globalThis.window;
  const originalDocument = globalThis.document;
  const originalNavigator = globalThis.navigator;
  ...
  try {
    ...
    await screen.findByText(/Tide outlook/i);
    assert.ok(screen.queryByText(/Provider alerts/i) === null);
  } finally {
    cleanup();
    globalThis.fetch = originalFetch;
    if (originalWindow === undefined) {
