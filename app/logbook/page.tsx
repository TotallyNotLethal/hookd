            ...
            <ul className="grid gap-4 md:grid-cols-2">
              {entries.map((entry) => (
                <li key={entry.id} className="rounded-3xl border border-white/10 bg-slate-900/60 p-4 space-y-2">
                  ...
                  {entry.length && (
                    <p className="text-xs text-white/60">Length: {entry.length} in</p>
                  )}
                  {entry.weight && (
                    <p className="text-xs text-white/60">Weight: {entry.weight} lb</p>
                  )}
                  ...
                  {entry.weather && <p className="text-xs text-white/50">Weather: {entry.weather}</p>}
                  {entry.notes && <p className="text-sm text-white/70">{entry.notes}</p>}
                </li>
              ))}
            </ul>
          )}
