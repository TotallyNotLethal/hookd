...
## Firebase Storage authorization troubleshooting

If uploads fail with `storage/unauthorized` and a 403 `Permission denied`, verify your Storage rules are not using the temporary time-based rule generated by Firebase Studio / console starter templates.
...
After that date passes, **all client uploads are denied** regardless of auth state.

This repo includes deployable rules in `storage.rules` and a `firebase.json` mapping for Storage. Deploy them with:
